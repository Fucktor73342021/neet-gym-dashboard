...
const LS_KEYS = {
  ...
  quote:    'nexus_quote',
  gymPhoto: 'gymPhoto',                        // Feature 1: Gym photo (base64 data URL)
  journal:  (dateKey) => `journal-${dateKey}`, // Feature 3: Daily journal (date-keyed text)
};
...
/* ─────────────────────────────────────────────────────────────────────────
   § 18 · GYM PROGRESS PHOTO UPLOAD
   ───────────────────────────────────────────────────────────────────────── */

/**
 * initGymPhoto()
 * Handles photo selection, FileReader preview, and localStorage persistence.
 * Uses element IDs:  gym-photo-input (file input)  &  gym-photo-preview (img)
 * Falls back safely if either element is absent.
 */
function initGymPhoto() {
  const fileInput = document.getElementById('gym-photo-input');
  const preview   = document.getElementById('gym-photo-preview');

  // Both elements must exist; abort silently if absent
  if (!fileInput || !preview) return;

  // ── Restore saved photo on page load ────────────────────────────────────
  const savedPhoto = localStorage.getItem(LS_KEYS.gymPhoto);
  if (savedPhoto) {
    preview.src           = savedPhoto;
    preview.style.display = 'block';
    const placeholder = document.getElementById('gym-photo-placeholder');
  ...
  fileInput.addEventListener('change', () => {
    ...
    if (!file.type.startsWith('image/')) {
      console.warn('[GymPhoto] Selected file is not an image:', file.type);
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      // Persist to localStorage (may throw if storage is full)
      try {
        localStorage.setItem(LS_KEYS.gymPhoto, dataURL);
      } catch (storageErr) {
        console.warn('[GymPhoto] Could not save to localStorage:', storageErr);
        alert('Photo displayed but could not be saved — storage may be full.');
      }
    };

    reader.onerror = () => {
      console.error('[GymPhoto] FileReader error for file:', file.name);
    };
...
/* ─────────────────────────────────────────────────────────────────────────
   § 19 · MULTI-MODE FOCUS TIMER
   ───────────────────────────────────────────────────────────────────────── */

/**
 * Timer mode configuration.
 * Each mode defines a label and duration in seconds.
 */
const TIMER_MODES = {
  study:      { label: 'STUDY SESSION',  seconds: 25 * 60 },
  rest:       { label: 'REST PERIOD',    seconds:  5 * 60 },
  gym:        { label: 'GYM SESSION',    seconds: 90       },
  meditation: { label: 'MEDITATION',     seconds: 10 * 60 },
};

/**
 * initTimer()
 * Wires up the Focus Timer section.
 * Element IDs used:
 *   timer-display  — shows MM:SS countdown
 *   start-btn      — begins / resumes the timer
 *   pause-btn      — pauses the timer
 *   reset-btn      — resets to the current mode's duration
 *   timer-mode     — <select> or hidden sync element for current mode
 * Gracefully exits if timer-display is absent.
 */
function initTimer() {
  const displayEl = document.getElementById('timer-display');
  if (!displayEl) return; // timer section not present in DOM
  ...
  let totalSeconds  = TIMER_MODES[currentMode].seconds;
  ...
  /** Render MM:SS into the display element */
  function renderDisplay() {
    const m = Math.floor(remainSeconds / 60);
    const s = remainSeconds % 60;
    displayEl.textContent = `${pad2(m)}:${pad2(s)}`;
  }

  /** Update the SVG progress ring (if present) */
  function renderRing() {
    const ring = document.getElementById('timer-progress-ring');
    if (!ring) return;
    const CIRC   = 326.7; // 2π × 52 (ring radius from HTML)
    const pct    = totalSeconds > 0 ? remainSeconds / totalSeconds : 0;
    const offset = CIRC - CIRC * pct;
    ring.style.strokeDashoffset = String(offset.toFixed(2));
  }

  /** Apply mode colours to display + ring */
  const MODE_COLORS = {
    study:      'var(--accent-cyan,#00e5ff)',
    rest:       'var(--accent-green,#00ff88)',
    gym:        '#ff6b35',
    meditation: 'var(--accent-purple,#a855f7)',
  };

  function applyModeColor(mode) {
    const color = MODE_COLORS[mode] || MODE_COLORS.study;
    ...
    if (modeLabel) {
      ...
      const m   = Math.floor(cfg.seconds / 60);
      const s   = cfg.seconds % 60;
      modeLabel.textContent = `${cfg.label} — ${pad2(m)}:${pad2(s)}`;
    }
  }

  /** Load a new mode (only allowed when timer is not running) */
  function loadMode(mode) {
    if (isRunning) return;
    currentMode   = mode;
    totalSeconds  = TIMER_MODES[mode].seconds;
  ...
  /** Stop the interval and mark as paused */
  function pauseTimer() {
    if (!isRunning) return;
  ...
  /** Log a completed or paused session to the session log panel */
  function logSession(mode, elapsedSeconds) {
    const logEl = document.getElementById('timer-session-log');
    if (!logEl) return;

    // Remove the placeholder message on first real entry
    const placeholder = logEl.querySelector('div');
    if (placeholder && placeholder.textContent.includes('NO SESSIONS')) {
      placeholder.remove();
    }

    const mins = Math.round(elapsedSeconds / 60);
    if (mins < 1) return; // don't log <1 min sessions

    const now  = new Date();
    const time = `${pad2(now.getHours())}:${pad2(now.getMinutes())}`;
    const color = MODE_COLORS[mode] || '#fff';
    const cfg   = TIMER_MODES[mode] || { label: mode.toUpperCase() };
    ...
    item.innerHTML = `
      ...
      <span style="font-family:var(--font-display,Orbitron,monospace);
                   font-size:0.75rem;color:${color};letter-spacing:2px;">
        ${mins}m &middot; ${time}
      </span>
    `;
    logEl.prepend(item);

    // Update total-today display
    updateTimerTotal(mins);
  }

  /** Accumulate minutes into the session-total display */
  function updateTimerTotal(addMinutes) {
    const totalEl = document.getElementById('timer-total-today');
    if (!totalEl) return;
    const current = parseInt(totalEl.textContent, 10) || 0;
    totalEl.textContent = `${current + addMinutes}m`;
  }

  // ── Initialise display with default mode ────────────────────────────────
  renderDisplay();
  renderRing();
  applyModeColor(currentMode);

  // ── Mode buttons (visual pill buttons) ──────────────────────────────────
  const modeBtns = document.querySelectorAll('.timer-mode-btn');
  modeBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      if (isRunning) return; // lock mode switch while running
      ...
      modeBtns.forEach(b => {
        ...
        b.classList.remove('is-active-mode');
      });
      const modeColor = MODE_COLORS[newMode] || 'var(--accent-cyan,#00e5ff)';
      btn.style.borderColor = modeColor;
      btn.style.color       = modeColor;
      btn.setAttribute('aria-pressed', 'true');
      btn.classList.add('is-active-mode');
  ...
  if (modeEl && modeEl.tagName === 'SELECT') {
    modeEl.addEventListener('change', () => {
      const chosen = modeEl.value;
      if (TIMER_MODES[chosen]) loadMode(chosen);
    });
  }
  ...
  if (startBtn) {
    startBtn.addEventListener('click', () => {
      if (isRunning) return;
      if (remainSeconds <= 0) return; // already finished, use reset first
      ...
      intervalId = setInterval(() => {
        ...
        if (remainSeconds === 0) {
          ...
          // Log the full session duration
          const elapsed = Math.round((Date.now() - sessionStart) / 1000);
          logSession(modeAtStart, elapsed);

          // Brief flash to signal completion
          displayEl.style.opacity = '0.4';
          setTimeout(() => { displayEl.style.opacity = '1'; }, 500);
        }
      }, 1000);
    });
  }
  ...
  if (pauseBtn) {
    pauseBtn.addEventListener('click', () => {
      ...
      pauseTimer();
      if (elapsed > 10) logSession(currentMode, elapsed);
    });
  }
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      pauseTimer();
      remainSeconds = totalSeconds; // reset to full duration of current mode
      renderDisplay();
      renderRing();
    });
  }
...
/* ─────────────────────────────────────────────────────────────────────────
   § 20 · DAILY JOURNAL SYSTEM
   ───────────────────────────────────────────────────────────────────────── */

/**
 * initJournal()
 * Auto-loads today's journal entry, saves on button click, renders past entries.
 * Element IDs used:
 *   journal-body   (textarea — primary input; also accepts "journal-input" fallback)
 *   save-journal   (save button)
 *   journal-title  (optional title input)
 *   journal-mood   (optional mood select)
 *   journal-date   (optional date input)
 * Falls back gracefully if elements are absent.
 */
function initJournal() {
  // Accept either ID used in the HTML
  ...
  const saveBtn  = document.getElementById('save-journal')
                || document.getElementById('journal-save-btn');

  // Neither required element present — exit silently
  if (!bodyEl || !saveBtn) return;
  ...
  const entryList  = document.getElementById('journal-entries-list');
  ...
  bodyEl.addEventListener('focus', () => {
    bodyEl.style.borderColor = 'var(--accent-green,#00ff88)';
  });
  bodyEl.addEventListener('blur', () => {
    bodyEl.style.borderColor = 'var(--border)';
  });

  // ── Helper: get all saved journal entries (array, newest first) ──────────
  function getAllEntries() {
    const raw = [];
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (!k || !k.startsWith('journal-')) continue;
      try {
        const parsed = JSON.parse(localStorage.getItem(k));
        // Normalise: entries can be a plain string (legacy) or object
        if (typeof parsed === 'string') {
          raw.push({ date: k.replace('journal-', ''), body: parsed, title: '', mood: 'focused' });
  ...
  // ── Mood colour map ──────────────────────────────────────────────────────
  const MOOD_COLORS = {
    focused:   'var(--accent-cyan,#00e5ff)',
    motivated: '#ff6b35',
    neutral:   'var(--text-muted)',
    tired:     'var(--accent-purple,#a855f7)',
    grinding:  '#ff3366',
  };

  // ── Render entries list panel ────────────────────────────────────────────
  function renderEntries() {
    ...
    if (!entries.length) {
      entryList.innerHTML = `
        <div style="font-family:var(--font-mono);font-size:0.6rem;color:var(--text-muted);
                    letter-spacing:1px;padding:20px;text-align:center;opacity:0.5;">
          NO ENTRIES YET — START WRITING, WARRIOR
        </div>`;
      return;
    }
    ...
    entries.forEach((entry) => {
      ...
      const moodColor = MOOD_COLORS[entry.mood] || 'var(--text-muted)';
      const displayTitle = entry.title ? entry.title : `ENTRY · ${entry.date}`;
        ...
        <div style="display:flex;justify-content:space-between;
          ...
          <div>
            <div style="font-family:var(--font-mono);font-size:0.65rem;
                        color:${moodColor};letter-spacing:1px;margin-bottom:2px;">
              ${displayTitle}
            </div>
            <div style="font-family:var(--font-mono);font-size:0.55rem;
                        color:var(--text-muted);letter-spacing:1px;">
              ${entry.date}&nbsp;&middot;&nbsp;${(entry.mood || 'focused').toUpperCase()}
            </div>
          </div>
          <button data-date="${entry.date}"
                  class="journal-del-btn"
                  aria-label="Delete journal entry from ${entry.date}"
                  style="background:transparent;border:1px solid var(--border);
                         color:var(--text-muted);font-size:0.6rem;cursor:pointer;
                         border-radius:2px;padding:2px 6px;flex-shrink:0;">
            ✕
          </button>
        </div>
        <div style="font-family:var(--font-body,Rajdhani,sans-serif);
                    font-size:0.82rem;color:var(--text-secondary,#aaa);
    ...
    // Delete button handlers (event delegation-style, attached after render)
    entryList.querySelectorAll('.journal-del-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const dateToRemove = btn.dataset.date;
        if (!dateToRemove) return;
        const confirmed = confirm(`Delete journal entry from ${dateToRemove}?`);
        if (!confirmed) return;
        lsRemove(LS_KEYS.journal(dateToRemove));
        renderEntries();
      });
    });
  }
  ...
  function loadTodayEntry() {
    const stored = lsGet(LS_KEYS.journal(todayKey));
    if (!stored) return;
    ...
    if (typeof stored === 'string') {
      ...
      if (titleEl && stored.title) titleEl.value = stored.title;
      if (moodEl  && stored.mood)  moodEl.value  = stored.mood;
    }
  ...
  saveBtn.addEventListener('click', () => {
    ...
    const entry = {
      date:  dateEl  ? (dateEl.value  || todayKey)     : todayKey,
      mood:  moodEl  ? (moodEl.value  || 'focused')    : 'focused',
      title: titleEl ? (titleEl.value.trim() || '')    : '',
      body,
    };

    lsSet(LS_KEYS.journal(entry.date), entry);
    ...
    if (savedMsg) {
      ...
      // Inline feedback on button if no dedicated message element
...
/* ─────────────────────────────────────────────────────────────────────────
   § 21 · NEW DAY DETECTION — clear today's data if it's a new day
   ───────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────
   § 23 · LIVE CLOCK — update topbar date string every minute
   ───────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────
   § 24 · EXERCISE ROW VISUAL SYNC on page load
   ───────────────────────────────────────────────────────────────────────── */
...
function boot() {
  ...
  // 6. New features — Photo · Timer · Journal
  initGymPhoto();
  initTimer();
  initJournal();

  // 7. Inject reset buttons
  initResetButtons(dateKey);

  // 8. Dashboard visual updates
  fullRefresh(dateKey);
  ...
  // 11. Periodic dashboard refresh every 5 minutes (keeps chart & streak up to date)
  setInterval(() => {
    updateDashboard(dateKey);
    renderWeeklyChart();
  }, 5 * 60 * 1000);
  ...
  console.log('%c Features: GymPhoto ✓  FocusTimer ✓  DailyJournal ✓',
